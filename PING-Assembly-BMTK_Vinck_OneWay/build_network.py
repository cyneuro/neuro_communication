...
def between_connector(source, target, p):
    global BETWEEN_COUNTER, MAX_BETWEEN, ALREADY_PROJECTING
    if source['node_id'] == target['node_id']: return 0
    if (BETWEEN_COUNTER < MAX_BETWEEN) and (np.sum(np.array(ALREADY_PROJECTING) == source['node_id']) < 3):
        ALREADY_PROJECTING.append(source['node_id'])
        BETWEEN_COUNTER += 1
        return 1
    else:
        return 0
...
if __name__ == "__main__":
    ...
    # Add between-assembly connections from assembly 1 to assembly 2
    source_assembly_id = 0
    target_assembly_id = 1
